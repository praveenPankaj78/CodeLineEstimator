...
public class ScanService {
	...
	public ScanResponse scan(ScanRequest request) {
		if(request.getLanguage().equalsIgnoreCase("java")) {
			...
			computeResponse(request.getFilePath(), cLang.getIMPORT(), cLang.getVARSYMBOLS(), cLang.getSINGLE_LINE_COMMENT_SYMBOL(), cLang.getMULTI_LINE_COMMENT_START(), cLang.getMULTI_LINE_COMMENT_END());
		}
...
/*					System.out.println(line.length());
					...
					if(line.contains(multiCommentStart)) {
						if(line.contains(multiCommentEnd) && (line.indexOf(multiCommentEnd) - line.indexOf(multiCommentStart)) > 0 ) {
							multiCommentFlag = false;
						}
						else {
							multiCommentFlag = true;
					...
					if(line.length() >= imports.length() && line.substring(0, imports.length()).equalsIgnoreCase(imports)) {
						++importLines;
					}
					
					
					/* Check if there is a variable declaration. 
					Please check Language class in util module for 
					list of supported data types. 
					This code checks variable type and = presence in the line with =
					being present after dataType.
					*/ 
					for(int j = 0; j < variableSymbols.size(); ++j) {
						if(line.contains(variableSymbols.get(j)) && line.contains("=") && (line.indexOf("=") - line.indexOf(variableSymbols.get(j)) > 0)) {
							++variableDecl;
							break;
						}
					}
